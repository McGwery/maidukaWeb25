...
class SendSubscriptionRenewedJob implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(): void
    {
        try {
            ...
            $text = "MaiDuka: Hongera! Mpango wako wa '{$planName}' kwa duka '{$shopName}' umehuishwa. Kiasi: TSh {$price}. Utaisha: {$expiryDate}. Asante kwa kuendelea kutumia MaiDuka!";

            Log::info("Sending subscription renewed SMS: {$text}");

            // Send SMS via Beem
            Beem::sms($text, [$senderPhone], "MaiDuka");

            Log::info("Subscription renewed notification sent to: {$owner->phone}");

        } catch (\Throwable $th) {
            Log::error("Failed to send subscription renewed notification: {$th->getMessage()}");
        }
    }
}
